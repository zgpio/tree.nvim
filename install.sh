...
get_latest_release() {
  fetch "https://api.github.com/repos/zgpio/tree.nvim/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  # mkdir -p bin
  # cd bin
  url="https://github.com/zgpio/tree.nvim/releases/download/$tag/${1}"
  info "Downloading binary from ${url}"
  if fetch "${url}" | tar xzfv - --strip-components 2; then
    # chmod a+x ${1%.tar.gz}
    return
  else
...
arch=$(uname -sm)
case "${arch}" in
  "Linux x86_64") download tree-linux.tar.gz ;;
  "Linux i686") download tree-linux.tar.gz ;;
  "Darwin x86_64") download tree-macos.tar.gz ;;
